...
| 参数 | 作用 | 建议值 |
|------|------|--------|
| `rotation_jitter=3.5` | 每字随机旋转 ±3.5°，这是让同字看起来不同的最主要因素 | 2~5，越大倾斜越明显 |
| `scale_jitter=0.05` | 每字随机缩放 ±5%，模拟下笔力度差异 | 0.03~0.08 |

#### 已有参数调整
| 参数 | 旧值 → 新值 | 原因 |
|------|-------------|------|
| `stroke_sigma` | 3.0 → 1.5 | 平滑半径变小，形变变局部，从"整体位移"变成真正的"笔画颤抖" |
| `stroke_alpha` | 4.5 → 5.0 | 配合更小的 sigma，维持可见的形变强度 |
